...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
        ...
        newt = [replace_leaf(tree, old, new) for tree in branches(t)]
...
def make_fib():
    ...
    index = 0
    def fib():
        nonlocal origional, index
        newfib = origional + index
        index += 1
        return newfib
    return fib
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    attemp = []
    def withdraw(amount, tempo):
        nonlocal balance, password, attemp
        if len(attemp) == 3:
            word = '"Your account is locked. Attempts:'
            quotes = '"'
            print(word,attemp,quotes)
            return None
        if amount > balance and tempo == password:
            return 'Insufficient funds'
        elif amount <= balance and tempo == password:
            balance = balance - amount
            return balance
        elif not(tempo == password) and len(attemp) < 4:
            attemp.append(tempo)
...
def generate_paths(t, x):
    ...
    if label(tree) == x:
        return [label(tree)]
    paths = [generate_paths(b,x) for b in branches(t)]
    for path in paths:
        if path:
            yield [label(tree)] + path
    return None
