    ...
    if expressions is nil:
        ...
        while not expressions is nil:
            if expressions.second == nil:
                return scheme_eval(expressions.first, env, expressions.second is nil)
            else:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        value = do_lambda_form(Pair(formals, expressions.second), env)
        env.define(function,value)
        return function
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        result = scheme_eval(expressions.first, env, expressions.second is nil)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(expressions.first, Pair) and scheme_symbolp(expressions.first.first):
        ...
        macro = MacroProcedure(expressions.first.second, expressions.second, env)
