...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn but it is useful becuase it has a large armor value"""
...
class ScubaThrower(ThrowerAnt):
    """A Schuba Thrower should not lose its armor when placed in water and is a subclass of ThrowerAnt"""
    name = 'Scuba';
    implemented = True
    food_cost = 6
    is_watersafe = True
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        position = self.place
        if isinstance(position.ant, BodyguardAnt) and position.ant not in QueenAnt.double:
            position.ant.damage *= 2
            QueenAnt.double.append(position.ant)
        """while position.entrance != None:
            if position.entrance.ant != None and position.entrance.ant not in QueenAnt.double:
                position.entrance.ant.damage *= 2
                if position.entrance.ant.container == True and not(position.entrance.ant.ant == None):
                    position.entrance.ant.ant.damage *= 2
                    QueenAnt.double.append(position.entrance.ant)
                QueenAnt.double.append(position.entrance.ant)
            pos = position.entrance

        position = self.place"""

        position = position.exit
        while position != None:
            if position.ant != None and position.ant not in QueenAnt.double:
                position.ant.damage *= 2
                QueenAnt.double.append(position.ant)
                if isinstance(position.ant, BodyguardAnt) and position.ant.contained_ant != None and position.ant.contained_ant not in QueenAnt.double:
                    position.ant.contained_ant.damage *= 2
                    QueenAnt.double.append(position.ant.contained_ant)
            if position.ant != None and position.ant in QueenAnt.double:
                if isinstance(position.ant, BodyguardAnt) and position.ant.contained_ant != None and position.ant.contained_ant not in QueenAnt.double:
                    position.ant.contained_ant.damage *= 2
                    QueenAnt.double.append(position.ant.contained_ant)
            position = position.exit
