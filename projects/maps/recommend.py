...
def find_closest(location, centroids):
    ...
    for x in centroids:
        if distance(location, x) < closest:
            closest = distance(location,x)
            centroid = x
    return centroid"""
    return min([x for x in centroids], key = lambda x: distance(location, x))
    """This is a better way to solve the problem since is more simple"""
    # END Question 3
...
def find_centroid(cluster):
    ...
    """get a list of locations of the restaurant in the cluster"""
    return[mean([x[0] for x in locations]), mean([x[1] for x in locations])]
    """return the mean of the first and second term in the list of location respectively"""
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        "*** YOUR CODE HERE ***"
        grouped_restaurant = group_by_centroid(restaurants, old_centroids)
        """group restaurants in clusters, where each cluster contains all restaurants closet to a particular centroid in centroids."""
        centroids = [find_centroid(r) for r in grouped_restaurant if len(r)!=0]
        """Update centroids to contain the true centroid for each cluster"""
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b, a, r_squared = sxy/sxx, meany - sxy/sxx * meanx, sxy**2/sxx/syy
